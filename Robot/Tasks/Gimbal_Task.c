...
void Yaw_Motor_Control(void)
{
    ...
    if (AutoAim_Data_Receive.yaw_aim != 0 || AutoAim_Data_Receive.pitch_aim != 0) // ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
    {
        ...
        gimbal_m6020[0].INS_speed_set = (-gimbal_m6020[0].auto_aim_pid.out) + (gimbal_m6020[0].INS_speed - gimbal_m6020[0].INS_speed_last) * YAW_MOTOR_AUTO_AIM_FF; // ï¿½ï¿½0.8ï¿½ï¿½Ä¿ï¿½ï¿½yawï¿½ï¿½ï¿½Ù¶È¡ï¿½Ç°ï¿½ï¿½
        gimbal_m6020[0].INS_angle_set = AutoAim_Data_Receive.yaw_aim;
        yaw_mode = yaw_mode_last = 1;
    }

    // Ò£ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ô¿ï¿½ï¿½ï¿½
    else if (rc_ctrl.rc.s[1] == RC_SW_MID)
    {
        ...
        else if (yaw_mode == 1 && yaw_mode_last == 0) // yawï¿½á¶¨ï¿½ï¿½ï¿½ï¿½×´Ì¬ï¿½Ð»ï¿½ï¿½Ä½Ç¶ï¿½
        {
    ...
    else if (rc_ctrl.rc.s[1] == RC_SW_UP) // ï¿½ï¿½Ê§Ä¿ï¿½ï¿½yawÍ£ï¿½ï¿½ï¿½ï¿½
    {
...
void Pitch_Motor_Control(void)
{
    ...
    else if (rc_ctrl.rc.s[1] == RC_SW_UP) // ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½pitchï¿½ï¿½ï¿½ï¿½Ò¡
    {
...
float Pitch_Updown(void)
{
    ...
    const PitchSwingParams swing_params[2] = {{-21.0f, -15.0f, 0.08f}, {3.0f, 20.0f, 0.05f}}; // Á½ÖÖpitch swingµÄ½Ç¶È£¬·Ö±ð¶ÔÓ¦´òÇ°ÉÚºÍÕý³£´òÈË
...
void DM_Auto_Enable()
{
    ...
    {
        enable_send_count = 5; // Ê¹ï¿½ï¿½ï¿½ï¿½ï¿?
        HAL_Delay(1000);
    }
