...
//		if(chassis_follow_gimbal_changing==1) //������Զ�޷��������������û����1�Ĵ���
//		{
//			yaw_mode=1;
//			yaw_mode_last=yaw_mode;
//		}
...
//			gimbal_m6020[0].INS_speed_set=100;
		...
		else if(yaw_mode==1&&yaw_mode_last==0)//��¼ң�����Ӹ�yaw�����ٶȵ������ٶȵ�һ˲���imu����
		{
			gimbal_m6020[0].INS_angle_set=gimbal_m6020[0].INS_angle;
		}
			
		if(yaw_mode==1)//ң���������룬yaw�ᶨ��
		{
...
//			if(gimbal_m6020[1].ENC_angle>5600&&gimbal_m6020[1].INS_speed_set>0)gimbal_m6020[1].INS_speed_set=0;
	...
	else if(rc_ctrl.rc.s[1]==RC_SW_UP) // �����������״̬������pitch����ҡ
	{
...
//void Yaw_Motor_Identification()//ϵͳ��ʶ����
//{
...
//		gimbal_m6020[0].give_current = gimbal_m6020[0].speed_pid.out;
//		CAN_Gimbal_CMD((int16_t)speed_set, gimbal_m6020[1].give_current, 0, 0);//�����Ϊ�����λ���?����̨������ƴ���
...
void Gimbal_Task(void const * argument)
{
	...
	//	angle_calculate();
		...
		Pitch_Updown();
		Pitch_Motor_Control();
		//Chassis_Gimbal_Angle_Tramsit.chassis_follow_gimbal_angle=DM_pitch_motor_data.pos; // �����������û�õ�
...
void enable_DM(uint8_t id, uint8_t ctrl_mode)
{
	...
	HAL_CAN_AddTxMessage(&hcan2, &Tx_Msg, TX_Data, &send_mail_box);    //�����ݴ��������FIFOx
}
